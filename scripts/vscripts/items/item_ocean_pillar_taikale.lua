LinkLuaModifier( "modifier_item_ocean_pillar", "items/item_ocean_pillar.lua", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_item_ocean_pillar_chain", "items/item_ocean_pillar.lua", LUA_MODIFIER_MOTION_NONE )
--Abilities
...
function modifier_item_ocean_pillar:OnAttackLanded(event)
    if IsServer() and not self.parent:IsIllusion() and event.attacker == self.parent and not event.target:IsNull() and event.target~= nil then
