darkness=class({})
LinkLuaModifier("modifier_darkness_buff",  "heroes/hero_night_stalker/darkness.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_darkness_debuff",  "heroes/hero_night_stalker/darkness.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_darkness_v",  "heroes/hero_night_stalker/darkness.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_darkness_sp",  "heroes/hero_night_stalker/darkness.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_darkness_buff:OnIntervalThink()
    if not IsServer() then
        ...
        AddFOWViewer(self:GetParent():GetTeamNumber(), self:GetParent():GetAbsOrigin(), self:GetAbility():GetSpecialValueFor("v"), 0.2, false)
end
...
function modifier_darkness_buff:GetModifierBaseDamageOutgoing_Percentage()
    local caster = self:GetCaster()
    return caster:GetMoveSpeedModifier(caster:GetBaseMoveSpeed(), true)* 0.15
end

function modifier_darkness_buff:GetModifierIgnoreMovespeedLimit() 
