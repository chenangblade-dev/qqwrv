...
LinkLuaModifier("modifier_hunter_in_the_night", "heroes/hero_night_stalker/hunter_in_the_night.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_hunter_in_the_night_ch", "heroes/hero_night_stalker/hunter_in_the_night.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_hunter_in_the_night:OnAttackLanded(tg)
    ...
    if tg.attacker == self:GetParent() and (not GameRules:IsDaytime()  or GameRules:IsNightstalkerNight() ) then
        ...
        local dam=tg.attacker:GetNightTimeVisionRange()*self:GetAbility():GetSpecialValueFor("dam") *0.01 
        local damage= {
            victim = tg.target,
...
function modifier_hunter_in_the_night:GetModifierMoveSpeedBonus_Percentage()
        ...
        elseif self:GetStackCount()==0 then
            return self:GetAbility():GetSpecialValueFor("sp")/2
        else 
...
function modifier_hunter_in_the_night:GetModifierAttackSpeedBonus_Constant()
        if self:GetParent():PassivesDisabled() then
            return 0
        elseif self:GetStackCount()==0 then
            return self:GetAbility():GetSpecialValueFor("attsp")/2
        else 
