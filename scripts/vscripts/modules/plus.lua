...
function Plus:OnPlayerPurchasePlus(keys)
	...
	local req = CreateHTTPRequestScriptVM('POST', GameRules.ECON_SERVER_URL .. '/PurchasePlus')	req:SetHTTPRequestGetOrPostParameter('steamid', tostring(steamid))
...
function Plus:QueryPlusDataFromServer()
	...
	local function queryPlusDataFromServer()
		local req = CreateHTTPRequestScriptVM('POST', GameRules.ECON_SERVER_URL .. '/QueryPlusData')
						...
						if PlayerResource:GetSteamAccountID(i) == tonumber(data.steamid) then
							...
							if data.is_vip == true then
								...
								if GameRules:State_Get() >= DOTA_GAMERULES_STATE_PRE_GAME then
									...
									gameEvent["locstring_value"] = string.format('%s.%s.%s', data.year, data.month, data.day)
									gameEvent["teamnumber"] = PlayerResource:GetPlayer(i):GetTeam()
									gameEvent["message"] = "#GameMessage_PurchasePlus_Success"
