...
-----------------------------------------------------------------------------------------------------------------------
-- 耀金羽翼
Econ.OnEquip_goldsky_wings_server = function(hero)
	if hero.nParticlegoldsky_wings then
		EconRemoveParticle(hero.nParticleLavaTrail)
	end
	hero.nParticlegoldsky_wings = EconCreateParticleOnHero(hero, "particles/wings/wing_sf_goldsky_gold.vpcf")
end

Econ.OnRemove_goldsky_wings_server = function(hero)
	if hero.nParticlegoldsky_wings then
		EconRemoveParticle(hero.nParticlegoldsky_wings)
	end
end
-----------------------------------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------------------------------
-- 彩虹嘲讽
Econ.OnEquip_rainbow_taunt_server = function(hero)
	hero.funcTaunt = function()
		local pid = ParticleManager:CreateParticle('particles/econ/taunts/rainbow.vpcf', PATTACH_ABSORIGIN_FOLLOW, hero)
		ParticleManager:SetParticleControlEnt(pid, 0, hero, PATTACH_ABSORIGIN_FOLLOW, 'follow_origin', hero:GetOrigin(), true)
		ParticleManager:ReleaseParticleIndex(pid)
	end
end
Econ.OnRemove_rainbow_taunt_server = function(hero)
	hero.funcTaunt = nil
end
-----------------------------------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------------------------------
-- 粒子之气
Econ.OnEquip_bbjl_server = function(hero)
	if hero.nParticlebbjl then
		EconRemoveParticle(hero.nParticlebbjl)
	end
	hero.nParticlebbjl = EconCreateParticleOnHero(hero, "particles/econ/events/golden_lotus_effect.vpcf")
end
Econ.OnRemove_bbjl_server = function(hero)
	if hero.nParticlebbjl then
		EconRemoveParticle(hero.nParticlebbjl)
	end
end
Econ.OnEquip_bbjl_client = Econ.OnEquip_bbjl_server
Econ.OnRemove_bbjl_client = Econ.OnRemove_bbjl_server
-----------------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------------------------------------------------------------
-- BAOJI VIP
Econ.OnEquip_juemingyiji_client = function(hero) end

Econ.OnRemove_juemingyiji_client = function(hero) end
Econ.OnEquip_juemingyiji_server = function(hero)
	hero.__bjuemingyiji = true
end

Econ.OnRemove_juemingyiji_server = function(hero)
	hero.__bjuemingyiji = false
end
-----------------------------------------------------------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------------------------------
-- 来自轮回谷的彩虹尾迹特效
Econ.OnEquip_rainbow_tail_server = function(hero)
	if hero.nParticleRainbowTail then
		EconRemoveParticle(hero.nParticleRainbowTail)
	end

	hero.nParticleRainbowTail = ParticleManager:CreateParticle("particles/imagine_assets/courier_fx/rainbow_tail.vpcf", PATTACH_CUSTOMORIGIN, hero)
	ParticleManager:SetParticleControlEnt(hero.nParticleRainbowTail, 0, hero, PATTACH_POINT_FOLLOW, "attach_hitloc", hero:GetAbsOrigin(), true)
end
Econ.OnRemove_rainbow_tail_server = function(hero)
	if hero.nParticleRainbowTail then
		EconRemoveParticle(hero.nParticleRainbowTail)
	end
end
...
-----------------------------------------------------------------------------------------------------------------------
-- 来自轮回谷的水幕特效
Econ.OnEquip_water_curtain_server = function(hero)
	if hero.nParticleWaterCurtain then
		EconRemoveParticle(hero.nParticleWaterCurtain)
	end

	hero.nParticleWaterCurtain = ParticleManager:CreateParticle("particles/imagine_assets/courier_fx/water_curtain.vpcf", PATTACH_ABSORIGIN_FOLLOW, hero)
	ParticleManager:SetParticleControlEnt(hero.nParticleWaterCurtain, 1, hero, PATTACH_POINT_FOLLOW, "attach_hitloc", hero:GetAbsOrigin(), true)
end
Econ.OnRemove_water_curtain_server = function(hero)
	if hero.nParticleWaterCurtain then
		EconRemoveParticle(hero.nParticleWaterCurtain)
	end
end
...
createEconLabel('summer_1')
createEconLabel('summer_2')
createEconLabel('summer_3')

function createEconEffect(name, path)
	Econ['OnEquip_' .. name ..'_server'] = function(hero)
		if hero['nEconParticle' .. name] then
			EconRemoveParticle(hero['nEconParticle' .. name])
		end
		hero['nEconParticle' .. name] = EconCreateParticleOnHero(hero, path)
	end
	Econ['OnRemove_' .. name .. '_server'] = function(hero)
		if hero['nEconParticle' .. name] then
			print("trying to remove econ particle")
			EconRemoveParticle(hero['nEconParticle' .. name])
		end
	end
	Econ['OnEquip_' .. name .. '_client'] = Econ['OnEquip_' .. name ..'_server']
	Econ['OnRemove_' .. name .. '_client'] = Econ['OnRemove_' .. name ..'_server']
end
-- 2021天陨旦纹章
createEconEffect('summer_2021', 'particles/econ/events/summer_2021/summer_2021_emblem_effect.vpcf')
