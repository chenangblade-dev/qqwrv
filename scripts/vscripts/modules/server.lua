GameRules.__NewAuthKey__ = GetDedicatedServerKeyV2('bom_23')
...
function Server:OnGetTop50()
    local request_str = GameRules.ECON_SERVER_URL .. "/GetTop50"
...
function Server:OnQueryPlayerRating()
    ...
    -- 发送请求去获取玩家的数据统计
    local req = CreateHTTPRequestScriptVM("POST", GameRules.ECON_SERVER_URL .. "/GetStastics")
    req:SetHTTPRequestGetOrPostParameter('player_json', player_json)
    req:Send(function(result)
			print('Server callback GetStastics')
            if result.StatusCode == 200 then
            local body = JSON:decode(result.Body)
...
function Server:OnGameStateChanged(something, debug)
    ...
    print("GameRules->", newState)
    if newState == DOTA_GAMERULES_STATE_CUSTOM_GAME_SETUP then
        Server:OnQueryPlayerRating()
        Server:OnGetTop50()
    end
    if newState == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS then
        local req = CreateHTTPRequestScriptVMWithKey( "POST", GameRules.ECON_SERVER_URL .. "/NewMatch")
        local players = {}
        for id = 0, DOTA_MAX_TEAM_PLAYERS do
        ...
        req:Send(function(result)
            if result.StatusCode == 401 then
                print('New Match with not enough players, aborting')
                return
            end
...
function Server:SendDataToServerAndEndGame()
    if GameRules.__RatingGameID__ == nil and not IsInToolsMode() then
        return
    end

    print('end game satrt!')
    local sortedTeams = {}
    ...
    local req = CreateHTTPRequestScriptVMWithKey( "POST", GameRules.ECON_SERVER_URL .. "/EndMatch")
    ...
    req:Send(function(result)
        DeepPrintTable(result)
			print('Server callback EndMatch')
            if result.StatusCode == 200 then
    ...
    local req1 = CreateHTTPRequestScriptVM('POST', GameRules.ECON_SERVER_URL .. '/SaveAbilityStats')
    req1:SetHTTPRequestGetOrPostParameter('abilities', JSON:encode(hero_ability_stats))
    req1:Send(function(result)
        print('Server callback SaveAbilityStats')
    end)
end
