...
local function getRandomBonus(starCount) -- 返回带有三位小数的属性增加值
	local bonus = math.round_off(
		-- 平均值 标准差 下限 上限
		RandomFloat(starCount / 2 - 0.7, starCount / 2 + 0.69), -- 7 - 8.5
		-3 -- 保留3位小数
	)
	if starCount > 8 then
		bonus = bonus + (starCount - 5) / 10 -- 最多额外提供1点
	end
	if starCount >= 15 and RollPercentage(50) then
		bonus = bonus + RandomFloat(0.5, 1)
	end
	return bonus
end
...
function Star:Init(hero)
	...
	function hero:GetCurrentStar()
		return hero.nCurrentStar
	end
