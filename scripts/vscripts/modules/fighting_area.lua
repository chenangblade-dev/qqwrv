...
function FightingArea:constructor()
	...
	self.vTimeMarks = {	13*60, 10*60, 5*60, } -- 这个时间是倒数的时间！也就是距离结束10分钟缩第一个圈！
...
function FightingArea:CreateTimer()
	Timer(1, function()
		...
		if countDown == timeMarks[3] then
			...
			-- 在第三次缩圈的时候，杀死所有信使
			if GameRules.vAllCouriers and table.count(GameRules.vAllCouriers) > 0 then
				for _, c in pairs(GameRules.vAllCouriers) do
					if IsValidAlive(c) then
						c:ForceKill(false)
					end
				end
			end
