...
function GameMode:Debug_OnPlayerChat(keys)
    ...
    if cmd == "center" then
        ...
        for _, hero in pairs(heroes) do
            FindClearSpaceForUnit(hero, Vector(-800, -300, 0), false)
            hero:Stop()
        end
    end
    ...
    if cmd == 'i' then
        ...
        if item == 'bbu' then
            for i = 1, count do
                hero:AddItemByName('item_spellbook_unlimited')
            end
        end
    end
    ...
    -- 开始刷怪
    if cmd == 'spawn' then
        GameRules.NeutralSpawner:PrepareSpawner()
    end

    if cmd == 'see' then
        AddFOWViewer(DOTA_TEAM_GOODGUYS, Vector(0,0,0), 9999, 9999, false)
    end
