...
LinkLuaModifier("modifier_multicast_lua", "abilities/hero_ogre_magi/ogre_magi_multicast_lua.lua", LUA_MODIFIER_MOTION_NONE)
...
if IsServer() then
	function modifier_multicast_lua:OnAbilityExecuted(keys)
		...
		if "item_ethereal_blade" == castedAbility:GetAbilityName() then
			multiplier = multiplier * 0.5
		end
	...
	local MULTICAST_ABILITIES = {
		...
		item_spellbook_ultimate  = MULTICAST_TYPE_NONE,
		item_spellbook_normal = MULTICAST_TYPE_NONE,
		item_spellbook_ultimate_courier = MULTICAST_TYPE_NONE,
		item_spellbook_normal_courier = MULTICAST_TYPE_NONE,
		item_dimensional_doorway = MULTICAST_TYPE_NONE,
