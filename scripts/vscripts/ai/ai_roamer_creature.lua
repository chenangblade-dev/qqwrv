...
AI.Init = function( self )
	...
	Timer(function() 
		...
		local tWaypoints = { self:GetAbsOrigin() }
		local nWaypointsPerRoamNode = 2
		local nMinWaypointSearchDistance = 100
		local nMaxWaypointSearchDistance = 2048

		for i = 1, nWaypointsPerRoamNode do
			local point = GameRules.GameMode:GetRandomValidPosition()
			table.insert( tWaypoints, 1, point)
		end
