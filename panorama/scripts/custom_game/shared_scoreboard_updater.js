...
function _ScoreboardUpdater_UpdatePlayerPanel(scoreboardConfig, playersContainer, playerId, localPlayerTeamId) {
    ...
    if (playerInfo) {
        ...
        var endRating = CustomNetTables.GetTableValue("end_game_rating", "end_game_rating");
        var steamid32 = -1;
        if (endRating != null) {
            for (var index in endRating) {
				var ratingInfo = endRating[index];
                var steamid = ratingInfo.steamid;
                var playerInfo = Game.GetPlayerInfo(playerId);
                steamid32 = playerInfo.player_steamid.substring(4);
                steamid32 = parseInt(steamid32) - 1197960265728;
                if (parseInt(steamid) == steamid32) {
					var rating = ratingInfo.rating;
                    var diff = ratingInfo.diff;
                    var match_count = ratingInfo.match_count;
                    if (match_count > 10) {
						if (diff >= 0) {
							_ScoreboardUpdater_SetTextSafe(playerPanel, "PlayerRating", rating + "+" + diff + "");
                            playerPanel.AddClass("RatingGreen");
                        } else {
							_ScoreboardUpdater_SetTextSafe(playerPanel, "PlayerRating", rating + diff);
                            playerPanel.AddClass("RatingRed");
                        }
                    } else {
						_ScoreboardUpdater_SetTextSafe(playerPanel, "PlayerRating", $.Localize("#need10game"));
                    }
                    break;
                }
            }
		...
		var playerAbilitiesContainer = playerPanel.FindChildInLayoutFile("AbilitiesContainer");
        if (playerAbilitiesContainer != null) {
            var heroIndex = Players.GetPlayerHeroEntityIndex(playerId);
            if (m_PlayerAbilitiesCache[playerId] == undefined) {
                m_PlayerAbilitiesCache[playerId] = {};
            }
            var bShouldUpdate = false;
            for (var i = 0; i < 11; i++) {
                var ability = Entities.GetAbility(heroIndex, i);
                if (Abilities.IsDisplayedAbility(ability)) {
                    if (m_PlayerAbilitiesCache[playerId][i] !== ability) {
                        bShouldUpdate = true;
                        break;
                    }
                }
            ...
            if (bShouldUpdate) {
                playerAbilitiesContainer.RemoveAndDeleteChildren();
                for (var i = 0; i < 12; i++) {
                    var ability = Entities.GetAbility(heroIndex, i);
                    var abilityPanel = $.CreatePanel("DOTAAbilityImage", playerAbilitiesContainer, "");
                    abilityPanel.abilityname = Abilities.GetAbilityName(ability);
                    m_PlayerAbilitiesCache[playerId][i] = ability;
                }
            }
        }
        ...
        if (starData == undefined || starData == null || !typeof starData == "object") {
            ...
            var data = starData[Players.GetPlayerHeroEntityIndex(playerId)];
            var starContainer = playerPanel.FindChildInLayoutFile("StarsContainer");
            if (starContainer !== null && data !== undefined) {
                var nCurrentStar = data.nCurrentStar;
                if (m_StarDataCache[playerId] !== nCurrentStar) {
                    m_StarDataCache[playerId] = nCurrentStar;
                    starContainer.RemoveAndDeleteChildren();
                    for (var i = nCurrentStar - 1; i >= 0; i--) {
                        var star = $.CreatePanel("Image", starContainer, "star_empty_" + i);
                        star.SetImage(isf);
                        star.style.width = "20px";
                        star.style.height = "20px";
                        star.style.verticalAlign = "center";
                    }
                }
            }
        }
